...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="panel panel-default">
            <div class="panel-heading">Handlers</div>
            <div class="panel-body">
                <div class="list-group">
                    @foreach ($handlers as $handler)
                    <a href="{{ route('handler.show', $handler->id) }}" class="list-group-item">
                        <img class="img-circle" src="../../assets/img/handler.png">
                        @if ($handler->status === "Active")

                        <span class="label label-success">{{ $handler->status }}</span>
                        &nbsp; <strong>{{ $handler->name }}</strong>

                        @else

                        <span class="label label-danger">{{ $handler->status }}</span>
                        &nbsp; <strong>{{ $handler->name }}</strong>
